...
const REPO_ROOT_MARKERS = ["package.json", ".ralph"];
const DEFAULT_ALLOWED_NPM_SCRIPTS = ["ci", "test:e2e", "typecheck", "build", "test:ci"];
...
export function findRepoRoot(start) {
    ...
    for (let i = 0; i < 10; i += 1) {
        if (REPO_ROOT_MARKERS.every((m) => existsSync(path.join(cur, m))))
            return cur;
...
export function findRepoRootForSetup(start) {
    let cur = path.resolve(start);
    for (let i = 0; i < 10; i += 1) {
...
export async function appendText(p, content) {
    await fs.mkdir(path.dirname(p), { recursive: true });
    const final = content.endsWith("\n") ? content : `${content}\n`;
    await fs.appendFile(p, final, "utf8");
}
export async function tailTextFile(p, n) {
    try {
...
export function parseFixPlan(markdown) {
    ...
    for (let idx = 0; idx < lines.length; idx += 1) {
        ...
        if (stripped.startsWith("#")) {
            ...
            if (header === "blocked" || header === "blocked tasks")
                section = "blocked";
            continue;
        }
...
export function setTaskChecked(markdown, ref, checked) {
    const lines = markdown.split(/\r?\n/);
...
export function blockTaskMd(markdown, ref, reason) {
    const lines = markdown.split(/\r?\n/);
    const tasks = parseFixPlan(markdown);
...
export function unblockTaskMd(markdown, ref) {
    const lines = markdown.split(/\r?\n/);
    const tasks = parseFixPlan(markdown);
...
export function replaceFixPlanMd(oldMd, newMd, preserveCompleted) {
    if (!preserveCompleted || !oldMd)
        return newMd.endsWith("\n") ? newMd : `${newMd}\n`;
    const oldTasks = parseFixPlan(oldMd);
    const newTasks = parseFixPlan(newMd);
    const oldById = new Map(oldTasks.filter((t) => t.taskId).map((t) => [t.taskId, t]));
    ...
    if (carriedCompleted.length || carriedBlocked.length) {
        ...
        if (carriedCompleted.length) {
            ...
            for (const t of carriedCompleted)
                newLines.push(`- [x] ${t.text}`);
        }
        if (carriedBlocked.length) {
            newLines.push("", "## Blocked", "");
            for (const t of carriedBlocked)
                newLines.push(`- [ ] ${t.text}`);
        }
    }
    const merged = newLines.join("\n");
    return merged.endsWith("\n") ? merged : `${merged}\n`;
}
export function getAllowedNpmScripts(root) {
    const allowed = new Set(DEFAULT_ALLOWED_NPM_SCRIPTS);
    ...
    try {
        ...
        if (cfg &&
            ...
            Array.isArray(cfg.allowedNpmScripts)) {
            for (const x of cfg.allowedNpmScripts)
...
function ensureBlockedSection(lines) {
    for (let i = 0; i < lines.length; i += 1) {
        const s = (lines[i] ?? "").trim().toLowerCase();
        if (s === "## blocked" || s === "## blocked tasks")
            return { updated: lines, blockedIndex: i };
    }
...
function insertIntoActive(lines, taskLine) {
