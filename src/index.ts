...
const tools: Tool[] = [
  {
    ...
    description:
      "Set up Phase 1: creates .ralph/phase1.sh and .ralph/phase1-prd-prompt.md, seeds a draft " +
    ...
    inputSchema: {
      ...
      properties: {
        sourcePlan: {
          type: "string",
          description:
            "Repo-relative path to the source plan markdown Phase 1 reads (seeded into config when config is merged). " +
  ...
  },
  {
    ...
    description:
      "Write or update a planning document under .github/plans/. " +
  ...
  },
  {
    ...
    description:
      "Write or replace the canonical product requirements document at .ralph/prd.md. " +
  ...
  },
  {
    ...
    description:
      "Read .ralph state: config, parsed tasks, spec index, PRD tail, and log tails. " +
  ...
  },
  {
    ...
    description:
      "List files and directories one level deep under a repo-relative path. " +
  ...
  },
  {
    ...
    description:
      "Read any text file inside the repo by its repo-relative path. " +
  ...
  },
  {
    ...
    description:
      "Write/update a spec file under .ralph/specs/**. " +
  ...
  },
  {
    ...
    description:
      "Replace .ralph/fix_plan.md, optionally preserving completed items from the previous file. " +
  ...
  },
  {
    ...
    description:
      "Check/uncheck a task in .ralph/fix_plan.md by text or taskId. " +
  ...
  },
  {
    ...
    description:
      "Set up Phase 2: .ralph/phase2.sh (planning persona + worker loop), config.json with defaults, " +
    ...
    inputSchema: {
      ...
      properties: {
        planFile: {
          type: "string",
          description:
            "Repo-relative path to the PRD or plan markdown (paths.projectPlan). " +
  ...
  },
  {
    ...
    description:
      "Set up Phase 3: .ralph/phase3.sh — Plan, Dev, QA personas, Dev/QA consensus sign-off, then QA close " +
  ...
  server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    switch (name) {
      case "ralph.generate_phase1":
        return jsonResult(
          await generatePhase1(args?.sourcePlan ? String(args.sourcePlan) : undefined),
        );
      ...
      case "ralph.write_plan":
        return jsonResult(
          await writePlan(String(args?.relativePath ?? ""), String(args?.content ?? "")),
        );
      ...
      case "ralph.upsert_spec":
        return jsonResult(
          await upsertSpec(String(args?.relativePath ?? ""), String(args?.content ?? "")),
        );

      case "ralph.replace_fix_plan":
        return jsonResult(
          await replaceFixPlan(String(args?.content ?? ""), Boolean(args?.preserveCompleted ?? true)),
        );
      ...
      case "ralph.unblock_task":
        return jsonResult(
          await unblockTask(String(args?.text ?? ""), asNullableString(args?.taskId)),
        );
      ...
      case "ralph.append_learning":
        return jsonResult(
          await appendLearning(String(args?.title ?? ""), String(args?.body ?? "")),
        );
