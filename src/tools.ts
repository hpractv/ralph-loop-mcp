...
import {
  ...
  blockTaskMd,
  findRepoRoot,
  findRepoRootForSetup,
  getAllowedNpmScripts,
  parseFixPlan,
  sandboxPath,
  setTaskChecked,
  tailTextFile,
...
import {
  PHASE1_PRD_PROMPT,
  PHASE2_PLANNER_PROMPT,
  PHASE2_WORKER_PROMPT,
  PHASE3_DEV_PROMPT,
  PHASE3_DEV_SIGNOFF_PROMPT,
  PHASE3_PLAN_PROMPT,
  PHASE3_QA_CLOSE_PROMPT,
  PHASE3_QA_PROMPT,
} from "./ralph-prompts.js";
...
async function listDir(dir: string, root: string) {
...
export async function writePlan(relativePath: string, content: string) {
...
export async function writePrd(content: string) {
  const root = findRepoRootForSetup(process.cwd());
...
async function ensurePromptFile(ralphDir: string, fileName: string, body: string) {
  const p = path.join(ralphDir, fileName);
  if (await fileExists(p)) return;
  await fs.writeFile(p, body.endsWith("\n") ? body : `${body}\n`, "utf8");
}

async function mergePhase1ConfigDefaults(ralphDir: string, defaultSource: string) {
  const configPath = path.join(ralphDir, "config.json");
  if (!(await fileExists(configPath))) return;
  let c: Record<string, unknown>;
  try {
    c = JSON.parse(await fs.readFile(configPath, "utf8")) as Record<string, unknown>;
  } catch {
    return;
  }
  ...
  c.phase1 = {
    ...
    planModel: typeof cur.planModel === "string" && cur.planModel.trim()
      ...
      : "claude-sonnet-4.6",
    planReasoningEffort:
      typeof cur.planReasoningEffort === "string" && cur.planReasoningEffort.trim()
...
export async function generatePhase1(sourcePlan?: string) {
  const root = findRepoRootForSetup(process.cwd());
...
export async function generatePhase2(planFile?: string) {
  const root = findRepoRootForSetup(process.cwd());
  const ralphDir = path.join(root, ".ralph");
  const configPath = path.join(ralphDir, "config.json");
...
export async function writeEpicPlan(content: string) {
  const root = findRepoRoot(process.cwd());
...
export async function upsertSpec(relativePath: string, content: string) {
  const root = findRepoRoot(process.cwd());
  const target = sandboxPath(root, relativePath);
  ...
  if (reservedSpecNames.has(baseName)) {
    throw new Error(
      "Reserved filename under .ralph/specs/. Use .ralph/logs/progress.txt and .ralph/logs/learnings.md for loop logs.",
    );
  }
...
export async function replaceFixPlan(content: string, preserveCompleted: boolean) {
  const root = findRepoRoot(process.cwd());
...
export async function nextTask() {
  ...
  if (!md) return { task: null, reason: "Missing .ralph/fix_plan.md" };

  const tasks = parseFixPlan(md);
  const t = tasks.find((x) => x.section === "active" && !x.checked);
  if (!t) return { task: null, reason: "No unchecked active tasks" };
  return { task: t };
}

export async function setTaskStatus(text: string, checked: boolean, taskId: string | null) {
  const root = findRepoRoot(process.cwd());
  const fixPlanPath = path.join(root, ".ralph", "fix_plan.md");
  const md = await fs.readFile(fixPlanPath, "utf8");
  const ref: TaskRef = { text, taskId };
  const updated = setTaskChecked(md, ref, checked);
  await fs.writeFile(fixPlanPath, updated, "utf8");
  return { ok: true };
}

export async function blockTask(text: string, reason: string, taskId: string | null) {
  const root = findRepoRoot(process.cwd());
  const fixPlanPath = path.join(root, ".ralph", "fix_plan.md");
  const md = await fs.readFile(fixPlanPath, "utf8");
  const ref: TaskRef = { text, taskId };
...
export async function unblockTask(text: string, taskId: string | null) {
  const root = findRepoRoot(process.cwd());
  const fixPlanPath = path.join(root, ".ralph", "fix_plan.md");
  const md = await fs.readFile(fixPlanPath, "utf8");
  const ref: TaskRef = { text, taskId };
...
export async function appendProgress(entry: string) {
  const root = findRepoRoot(process.cwd());
...
export async function appendLearning(title: string, body: string) {
  const root = findRepoRoot(process.cwd());
...
export async function runVerification(timeoutSeconds: number) {
  const root = findRepoRoot(process.cwd());

  const results: Array<{ script: string; returncode: number; stdout: string; stderr: string }> = [];
  for (const script of ["ci", "test:e2e"] as const) {
...
export function validateRalphConfig(cfg: unknown): { ok: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  if (phase3c) {
    for (const key of ["planModel", "devModel", "qaModel"] as const) {
...
function getConfiguredProjectPlanPath(cfg: unknown): string | null {
  const root = asObject(cfg);
  const paths = root ? asObject(root.paths) : null;
  if (!paths || typeof paths.projectPlan !== "string") return null;
 ...
 */
function extractPhase2PlanningTasks(fixPlanMd: string): string[] {
 ...
 */
async function buildPhase3SeedTasks(ralphDir: string, fixPlanMd: string): Promise<string[]> {
  ...
  for (const t of extractPhase2PlanningTasks(fixPlanMd)) add(t);
  ...
  if (phase3PlanMd) {
    for (const t of extractPhase2PlanningTasks(phase3PlanMd)) add(t);
 ...
 */
function appendPhase3Tasks(existing: string, taskTexts: string[]): string {
...
async function listMarkdown(dir: string, root: string) {
  try {
